...
exports.dbFindToken = async function(token) {
    const sql = (`SELECT tokenId FROM ${tableTokens} WHERE token = '${token}'`);
    let results = await pool.query(sql);
    let foundResults = results.length;
    let valid = (foundResults === 1);
    let sessionUserId = results.userId;
...
exports.dbInsertClient = async function(newClient) {
    ...
    if (ifName.length != 0) { console.log("NAME EXIST"); return ("שם משתמש תפוס"); } else if (ifNick.length != 0) { console.log("NICK EXIST"); return ("כינוי תפוס"); } else if (ifAccount.length != 0 && account != 0) { console.log("ACCOUNT EXIST"); return ("מספר חשבון תפוס"); } else {
        let messageReturn;
